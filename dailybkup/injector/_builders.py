...
class StorerBuilder():
    ...
    def __init__(
            ...
            l_backup_file_name_generator: Callable[[str], storermod.IBackupFileNameGenerator],
    ):
        self._l_b2context = l_b2context
    ...
    def build(self, config: configmod.IStorageConfig) -> storermod.IStorer:
        ...
        if isinstance(config, configmod.B2StorageConfig):
            b2context = self._l_b2context(config.bucket)
            backup_file_name_generator = self._l_backup_file_name_generator(config.suffix)
...
class EncryptorBuilder():
    ...
    def build(self, config: Optional[configmod.IEncryptionConfig]) -> encryptionmod.IEncryptor:
        if config is None:
